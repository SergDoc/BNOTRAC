 ...
 * isFullyCalibrated
 */
void getQuat(uint8_t vector_type, float* linacc){
	/*
	...
	switch(vector_type){
    	...
    	case VECTOR_QUAT:
    		temp[0] = (((int16_t)bufferquat[0]) | (bufferquat[1]) << 8);
    		temp[1] = (((int16_t)bufferquat[2]) | (bufferquat[3]) << 8);
    		temp[2] = (((int16_t)bufferquat[4]) | (bufferquat[5]) << 8);
    		temp[3] = (((int16_t)bufferquat[6]) | (bufferquat[8]) << 8);

    		/* 1 degree = 16 LSB */
    		linacc[0] = ((float)temp[0])/16384.0f;;
    		linacc[1] = ((float)temp[1])/16384.0f;
    		linacc[2] = ((float)temp[2])/16384.0f;
    		linacc[3] = ((float)temp[3])/16384.0f;
    		break;
...
int8_t I2C_read8(uint8_t slave, uint8_t address){
	...
	HAL_Delay(20);
	HAL_I2C_Master_Receive(&hi2c2,slave,(uint8_t *)&val, 1, 100);
	return val;
}
